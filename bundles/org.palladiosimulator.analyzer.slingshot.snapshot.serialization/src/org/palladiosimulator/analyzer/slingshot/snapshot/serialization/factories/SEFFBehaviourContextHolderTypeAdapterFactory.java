...
public class SEFFBehaviourContextHolderTypeAdapterFactory implements TypeAdapterFactory {
	 ...
	 */
	private final Map<TypeToken<SeffBehaviorContextHolder>, TypeAdapter<SeffBehaviorContextHolder>> thisAdapter = new HashMap<>();
	...
	public final <T> TypeAdapter<T> create(final Gson gson, final TypeToken<T> type) {
		if (SeffBehaviorContextHolder.class.isAssignableFrom(type.getRawType())) {
			...
			if (!thisAdapter.containsKey(type)) {
				thisAdapter.put((TypeToken<SeffBehaviorContextHolder>) type, customizeMyClassAdapter(gson, (TypeToken<SeffBehaviorContextHolder>) type));
			}
