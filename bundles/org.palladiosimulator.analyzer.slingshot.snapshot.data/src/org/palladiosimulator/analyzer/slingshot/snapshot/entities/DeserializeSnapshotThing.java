 ...
 *
 */
public final class DeserializeSnapshotThing{

	protected static final Logger LOGGER = Logger.getLogger(DeserializeSnapshotThing.class);
	...
	public DeserializeSnapshotThing(final PCMResourceSetPartition partition) {
	...
	public Snapshot deserializeFromFile(final Path path) {
		final String read = read(path.toFile());
		return gson.fromJson(read, Snapshot.class);
	}
	
	public void serializeToFile(final Snapshot snapshot, final Path path) {
		final String json = gson.toJson(snapshot);
		this.write(json, path.toFile());
	}
	...
	private void write(final String string, final File file) {
		LOGGER.debug("save json to " + file.toString());
	...
	private class SnapshotSerializer implements JsonSerializer<Snapshot> {
		...
		public JsonElement serialize(final Snapshot src, final Type typeOfSrc, final JsonSerializationContext context) {
			...
			final Set<DESEvent> events = src.getPlainEvents();
