...
public record StateGraphNode(String id, double startTime, double endTime, List<MeasurementSet> measurements,
	...
	private static Utility calcUtility(final double startTime, final double endTime, final List<MeasurementSet> measurements,
		...
		for (final ServiceLevelObjective slo : slos) {
			final MeasurementSet ms = measurements.stream()
					...
					.orElse(null);
			if (ms != null) {
				final var value = calculateAreaUnderCurveMeasure(ms, startTime, slo);
