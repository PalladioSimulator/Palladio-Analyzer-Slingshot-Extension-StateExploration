...
public class Utility {
	...
	public void calculateTotalUtility() {
		...
		for(int i = 0; i < length; i++) {
			...
			final var timeStampMax = List.of(costIntervals.stream().mapToDouble(x -> x.get(a).timeStamp()).max(),
					sloIntervals.stream().mapToDouble(x -> x.get(a).timeStamp()).max()
					).stream().mapToDouble(x -> x.orElse(0.0)).max().orElse(1.0);
			final var timeStampMin = List.of(costIntervals.stream().mapToDouble(x -> x.get(a).timeStamp()).min(),
	...
	private static List<List<Measurement<Double>>> computeIntervals(final double start, final double end, final List<List<Measurement<Double>>> list) {
	...
	private static List<Measurement<Double>> computeIntervalList(final double start, final double end, final List<Measurement<Double>> list) {
