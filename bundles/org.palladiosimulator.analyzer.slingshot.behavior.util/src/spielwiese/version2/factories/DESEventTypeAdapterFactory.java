...
public class DESEventTypeAdapterFactory implements TypeAdapterFactory {
	...
	/**
	 * According to Doc, creating delegators is costly, thus we save them, if possible. 
	 */
	...
	private TypeAdapter<DESEvent> customizeMyClassAdapter(final Gson gson, final TypeToken<DESEvent> type) {
		final TypeAdapter<DESEvent> delegate = gson.getDelegateAdapter(this, type); // for writing only.
		...
		return new TypeAdapter<DESEvent>() {
			...
			public DESEvent read(final JsonReader in) throws IOException {
				...
				final JsonPrimitive type = this.getFieldAs(parent,  JsonElement::isJsonPrimitive, JsonElement::getAsJsonPrimitive, FIELD_NAME_CLASS);
				final JsonObject event = this.getFieldAs(parent,  JsonElement::isJsonObject, JsonElement::getAsJsonObject, FIELD_NAME_EVENT);

				try {
					final var clazz = Class.forName(type.getAsString());
					
					final TypeAdapter<DESEvent> adapter = ReifiedEvent.class.isAssignableFrom(clazz) ? createSpecialAdapter(clazz, event) : createSimpleAdapter(clazz);
			...
			private TypeAdapter<DESEvent> getDelegatAdapter(final TypeToken<?> tt) {
				if (!eventDelegators.containsKey(tt)) {
					eventDelegators.putIfAbsent(tt, (TypeAdapter<DESEvent>) gson.getDelegateAdapter(forReference, tt));
				}
			 ...
			 * @return
			 */
			private TypeAdapter<DESEvent> createSpecialAdapter(final Class<?> eventClass, final JsonObject jsonObj) {
			 ...
			 * @return
			 */
			private <T extends JsonElement> T getFieldAs(final JsonObject json, final Predicate<JsonElement> criteria, final Function<JsonElement, T> accessor, final String fieldName) {
				if (!json.has(fieldName) ||  json.get(fieldName) == null || !criteria.test(json.get(fieldName))) {
					throw new JsonParseException(
							String.format("Expected JsonObjects with non-null field \"%s\" , but found %s.",
									 fieldName, json));
				}
